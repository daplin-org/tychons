...
The two-word checksum phrase at the bottom of every Tychon Badge is drawn from a wordlist. The choice of wordlist affects the entropy of the phrase and the language in which it is displayed.
...
BIP-39 is a Bitcoin standard that defines 2048-word lists in multiple languages. Tychon Badge adopts these lists because they are widely available, carefully curated, and standardized.
...
Lines beginning with `#` and blank lines are ignored. The file should contain exactly 2048 words for a standard BIP-39 list, though Tychon Badge will accept lists of other lengths.
...
When no language is specified, Tychon Badge uses a hardcoded 64-word English list:
...
Two Tychon Badge implementations will produce the same badge for the same key **only if they use the same wordlist** (identical words in identical order). If you share badges across systems or teams, standardize on a specific wordlist file and distribute it alongside the implementation.
